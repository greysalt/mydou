  ...
  <div class="card-container">
    <div class="md-layout md-alignment-left">   
      <div class="md-layout-item md-size-33" v-for="item in subjects" :key="item.id">
        <md-card class="card">
          <md-ripple>
            <router-link class="card-link" :to="{ path: type + '/detail/' + item.id }">
              <md-card-media>
                <div class="card-img" :style="{ backgroundImage: 'url(' + item.img + '),url(/mydou/dist/static/img/book_default.png)' }"></div>
              </md-card-media>
              <div class="card-info">
    ...
    <div class="nomore-info" v-if="!subjects.length">- 还没有收藏的内容哦 -</div>
  </div>
</template>
<script>
